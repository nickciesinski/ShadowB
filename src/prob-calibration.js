'use strict';
// =============================================================
// src/prob-calibration.js — put the model's output on a probability scale
//
// NOT to be confused with src/calibration.js, which is a different thing that
// already existed: a unit-SIZING feedback loop (getCalibrationMultiplier) that
// game-model.js depends on. This module was briefly written over that file on
// 2026-08-31 and broke trigger4 for ~2 runs. Hence the distinct name.
//
...
function loadCalibration(league, dir) {
  ...
  const file = path.join(dir || path.join(__dirname, '..', 'config'), `prob-calibration.${key}.json`);
  let cfg = null;
  try {
