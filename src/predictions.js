 ...
 * Local alias — canonical version lives in market-pricing.js.
 */
...
async function logPicksToPerformanceLog(picks, sport, oddsRows, weights) {
  ...
  for (const p of picks) {
    ...
    // ── Legacy GPT pick path (kept for backward compat) ──
...
async function writeApprovedToDailyCombos(picks, sport) {
  ...
  const rows = approved.map(p => [
    ...
    '',                            // away — filled at email time from perf log
    '',                            // home — filled at email time from perf log
    p.team || '',
    p.line || '',
    p._odds || -110,
    p._units || 0,
